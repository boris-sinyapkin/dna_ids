...
def get_best_signature(aligner : Align.PairwiseAligner, normal_act : list) -> dict:
    
    assert(normal_act != [])

    print("Ideal sequence searching...")
    print("Amount of normal activities: %d" % (len(normal_act)))

    with open(Path("matrix1.csv"), 'w') as csv_file:
        ...
        for seq_rec in normal_act:       
            ...
            if align_info.get_sum() > max_sum:
                ...
                max_sum  = align_info.get_sum()  

                print("New candidate ID: %s" % (seq.id)) 
